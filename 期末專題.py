...
ray = sphere(pos = vec(-6, 1, 0), color = color.blue, radius = 0.05, make_trail=True)
ray.v = vector(lightspeed * cos(theta_0), lightspeed * sin(theta_0), 0)
...
v1 = lightspeed
v2 = lightspeed
n_prev = n_air = 1 + (n_ref - 1) * pressure_0 / (1 + (temperature_0 - 15) * (0.0034785))
while True:
    rate(5000)
